...
let state = {
    ...
    score: 0,
    lastDate: null,
    goals: { water: 2.0, steps: 8000, rest: 8 },
    myRoutine: [],
    streak: 0,
    totalProductsUsed: 0,
    dailyHistory: [],
...
function handleAppClick(event) {
    ...
    switch (action) {
        ...
        case 'add-to-routine':
            ...
            break;
        case 'confirm-add-routine':
            confirmAddRoutine(trigger.dataset.id, trigger.dataset.time);
            break;
    }
}
...
function incrementSteps(event) {
    state.steps = Math.min(50000, state.steps + 500);
        ...
        <div class="text-center space-y-6">
            ...
            <div class="space-y-3">
                ...
                <div class="grid grid-cols-3 gap-3">
                    <button data-action="confirm-add-routine" data-id="${prodId}" data-time="Mañana" class="p-4 rounded-2xl border-2 border-emerald-300 bg-emerald-50 text-emerald-700 font-bold text-sm btn-press transition-all hover:bg-emerald-100">
                        🌅 Mañana
                    </button>
                    <button data-action="confirm-add-routine" data-id="${prodId}" data-time="Tarde" class="p-4 rounded-2xl border-2 border-blue-300 bg-blue-50 text-blue-700 font-bold text-sm btn-press transition-all hover:bg-blue-100">
                        ☀️ Tarde
                    </button>
                    <button data-action="confirm-add-routine" data-id="${prodId}" data-time="Noche" class="p-4 rounded-2xl border-2 border-indigo-300 bg-indigo-50 text-indigo-700 font-bold text-sm btn-press transition-all hover:bg-indigo-100">
                        🌙 Noche
                    </button>
                </div>
            </div>
...
function confirmAddRoutine(prodId, time) {
    const prod = products.find(p => p.id === prodId);
    state.myRoutine.push({ ...prod, completed: false, time });
    logActivity('routine-add', `Agregaste ${prod.name} para ${time}`);
    saveState();
    closeModal();
    showModal('✨', 'Perfecto', `${prod.name} se ha sumado a tu rutina diaria por la ${time}.`);
}
...
function updateGoals() {
    ...
    state.goals.water = Math.max(0.5, Math.min(5, waterVal || 2.0));
    state.goals.steps = Math.max(1000, Math.min(50000, stepsVal || 8000));
    saveState();
}
...
function calculateBrioScore() {
    ...
    const routineP = state.myRoutine.length > 0
        ? (state.myRoutine.filter(r => r.completed).length / state.myRoutine.length) * 100
        : 100;
...
function renderAll() {
    ...
    document.getElementById('steps-val').textContent = (state.steps / 1000).toFixed(1) + 'k';
    document.getElementById('steps-goal-val').textContent = (state.goals.steps / 1000).toFixed(0) + 'k';
    const tipIndex = new Date().getDate() % state.dailyTips.length;
    ...
    routineList.innerHTML = state.myRoutine.length === 0
        ? `<div class="p-12 text-center border-2 border-dashed border-slate-200 rounded-[2.5rem] text-slate-400 font-medium">
                <i class="fas fa-plus-circle text-3xl mb-4 block text-slate-300"></i>
                Añade productos desde Fórmulas para armar tu rutina.
           </div>`
        : state.myRoutine.map(item => `
                    ...
                    <div class="flex items-center justify-between gap-4">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                ${item.completed ?
                                    `<p class="text-[10px] font-bold text-emerald-500 uppercase tracking-widest">Completado</p>` :
                                    `<div class="space-y-1">
...
function exportData() {
    ...
    a.href = url;
    a.download = `briovida-data-${new Date().toISOString().split('T')[0]}.json`;
...
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        // Note: Would register a service worker here for offline functionality
    });
}
